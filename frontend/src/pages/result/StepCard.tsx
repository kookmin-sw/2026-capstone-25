  ...
  return (
    <div className="bg-sf border border-bd2 rounded-xl px-4 py-[14px] mb-[10px]">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="w-full flex items-center gap-[10px] bg-transparent border-none p-0 cursor-pointer text-left"
      >
        <div className="w-6 h-6 rounded-full flex-shrink-0 bg-ac-s text-ac-d flex items-center justify-center text-[11px] font-extrabold">
          {index + 1}
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-bold text-tx truncate">{step.title}</div>
          {step.description && (
            <div className="text-[11px] text-mu mt-[2px] line-clamp-1">{step.description}</div>
          )}
        </div>
        <span className="flex-shrink-0 text-[11px] text-tx2 font-semibold bg-fa border border-bd2 rounded-full px-[10px] py-[3px]">
          {step.estimated_minutes}분
        </span>
        <span
          aria-hidden
          className={[
      ...
      {open && (
        <div className="mt-[10px] pt-[10px] border-t border-bd2 flex flex-col gap-2">
          <GuideRow label="산출물" value={step.guide} fallback="산출물이 비어 있어요." />
          <GuideRow label="첫 동작" value={step.first_move} fallback="첫 동작이 비어 있어요." />
          <GuideRow label="막힘 대응" value={step.unblocker} fallback="막힘 대응이 비어 있어요." />
        </div>
      )}
    </div>
  );
