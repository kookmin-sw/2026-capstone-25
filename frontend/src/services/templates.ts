...
export const TEMPLATES: Template[] = [
  ...
  // ── 업무 ─────────────────────────────────────────────
  {
    ...
    structureHint:
      "사이드 프로젝트는 기획 → 설계 → MVP 구현 → 공개·배포 → 다듬기 흐름이다. mode 전환(생각하기/만들기/내보내기)이 강한 경계. 종류에 따라 구현 단계의 입자도가 다르므로 customFields 의 종류와 핵심 기능을 보고 분해하라. 주 가용 시간이 명시되면 그것으로 단계 크기 상한을 가늠한다. 미정인 부분이 많으면 의사결정을 작게 분리한다.",
  ...
  },
  {
    ...
    icon: "📋",
    customFields: [
      "- 회의 종류 (1on1 / 팀 정기 / 킥오프 / 워크숍 / 고객 미팅):",
      "- 회의 시간 (15분 / 30분 / 1시간 / 그 이상):",
      "- 도착해야 할 결정 (가장 중요):",
    ],
    structureHint:
  ...
  },
  {
    ...
    icon: "📑",
    customFields: [
      "- 종류 (보고서 / 제안서 / 매뉴얼 / 기획서 / 기타):",
      "- 대상 독자 (직위·전문성·관심사):",
      "- 핵심 메시지·결론 한 줄 (있다면):",
    ],
    structureHint:
      "업무 문서는 목적·결론 정의 → 자료·근거 모으기 → 목차 → 초안 → 다듬기 → 검토·공유 흐름이다. 결론을 먼저 정하고 본문은 그것을 뒷받침하는 방향으로 분해한다. 대상 독자의 전문성·관심사에 따라 깊이·용어를 조정한다.",
  ...
  },
  {
    ...
    icon: "🧑‍💼",
    customFields: [
      "- 문항별 글자수 (예: 지원동기 500자 / 성장과정 800자):",
      "- 강조하고 싶은 경험 (구체적 사건·결과):",
      "- 회사가 중시하는 키워드 (인재상·핵심가치·JD):",
    ],
    structureHint:
      "자기소개서는 경험 정리 → 문항 분석 → 경험·문항 매핑 → 문항별 초안 → 다듬기 흐름이다. 문항 단위로 의미가 다르므로 문항별로 끊는다. 같은 경험을 여러 문항에 재활용할 때는 강조점을 다르게 잡는 단계를 분리한다. 회사 키워드가 명시되면 본문 작성·다듬기에 반영한다.",
    previewChunks: ["회사·JD 분석", "경험 정리", "문항 매핑", "문항별 초안", "톤 다듬기"],
    featured: true,
  },
  {
    id: "career-multi-apply",
    category: "career",
    name: "입사 동시 지원",
    icon: "📨",
    customFields: [
      "- 지원할 회사 목록 (이름·마감일):",
      "- 회사별 우선순위 (1지망 / 2지망 / 보험용):",
      "- 지원 기간 중 가용 시간 (주 N시간):",
    ],
    structureHint:
      "동시 지원은 공통 자료 준비 → 회사별 분기 → 회사별 맞춤화 → 제출 → 추적·후속 흐름이다. '공통→개별 분기'가 강한 의미 경계. 회사 수가 많으면 회사 단위로 자식 단계가 자연스럽지만, 1차 단계는 공통/맞춤/제출/추적의 phase 로 끊는다. 우선순위가 높은 회사에 시간 비중을 더 둔다.",
    previewChunks: ["회사 목록·마감", "공통 자료", "회사별 맞춤화", "제출", "추적·후속"],
  },
  {
    id: "career-exam",
    category: "career",
    ...
    customFields: [
      "- 시험 과목과 출제 비중:",
      "- 현재 진척도 (1회독 끝 / 기출 풀이 중 / 모의 단계):",
      "- 약한 과목·단원:",
    ],
    structureHint:
      "채용 시험은 시험일 역산 → 과목별 진도 → 기출 회독 → 약점 보강 → 모의·최종 점검 흐름이다. dependency(과목 간 선후)와 phase(이론→기출→마무리)가 강한 경계다. 시험까지 남은 일수와 현재 진척도가 phase 비율을 결정한다. 장기 사이클이므로 1차 단계는 phase 로 크게 잡고 자식 단계로 세분화한다.",
  ...
  },
  {
    ...
    icon: "📁",
    customFields: [
      "- 담을 프로젝트·작업물 후보 (제목 5개 내외):",
      "- 각 프로젝트에서 내 역할 (개인 / 리딩 / 참여):",
      "- 형식 (PDF / 웹 / 노션):",
    ],
    structureHint:
  ...
  // ── 생활 ─────────────────────────────────────────────
  {
    ...
    customFields: [
      ...
      "- 부양가족 여부와 형태 (배우자·자녀·부모):",
      "- 신경 쓰이는 공제 항목 (의료비·교육비·기부·월세·청약·연금저축 등):",
    ],
    structureHint:
