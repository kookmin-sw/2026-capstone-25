...
- immediacy — 지금 · 혼자 · 바로 시작 가능. 외부 대기(피드백·승인·결과 통보 등)는 별도 단계로 만들지 말고, 대기를 유발하는 사용자 액션 단계의 description에 후속 행동을 적어 흡수한다. 예: "초안을 보낸 뒤 3일 안에 답이 없으면 재발송."
...
- 입력 task를 phase 단위로 끊는다. 각 단계는 그 자체로 작은 프로젝트일 수 있다.
- 권장 단계 수: 3~7.
- 단계 당 estimated_minutes는 60분부터 수십 시간까지 자유. 시간 약속을 지키느라 phase를 인위적으로 쪼개지 마라.
- 추가 분해가 필요한 단계는 boundary_signal을 "phase"로 부여해 신호한다.
- parent_step_id는 모두 null.

## 2차 분해 (부모 단계 있음)

- 부모 단계 하나를 즉시 실행 가능한 단위로 끊는다.
- 권장 단계 수: 3~9.
- 단계 당 estimated_minutes는 15~90분. 90분을 초과하면 더 쪼개라.
- "한발짝"의 약속(작고 즉시 시작 가능)은 이 단계에서 지킨다.
- parent_step_id는 모두 입력으로 받은 부모 step_id로 채운다.
- analysis 블록은 부모의 맥락을 그대로 사용해도 된다.
