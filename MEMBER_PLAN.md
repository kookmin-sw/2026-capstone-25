...
**산출물**:
- [ ] `frontend/src/App.tsx` (라우팅 5개)
- [ ] `components/AppShell.tsx` (반응형 좌측 사이드바 ↔ 하단 네비)
- [ ] `components/Header.tsx` (로고 + "오늘 0분" 필)
- [ ] 5개 페이지 placeholder (`HomePage`, `CalendarPage`, `AllPage`, `ReportPage`, `MePage`)
...
**산출물**:
- [ ] `pages/HomePage.tsx` (직접 입력/템플릿 탭 토글)
- [ ] `components/InputForm.tsx` (제목·메모·시작/마감일·AI 토글 + react-hook-form + zod)
- [ ] `services/decompose.ts` (mock 응답: setTimeout 2초 → 하드코딩 JSON)
- [ ] 로딩 progress 4단 (INTAKE/DECOMPOSE/VALIDATE/PRESENT)
...
**총 대략 40개 신규 파일.** 바이브 코딩이라 Claude가 대부분 작성하고 사람은 검수·수정 위주.
